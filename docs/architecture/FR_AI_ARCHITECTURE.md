...
BALANCE AUTHORITY
- Codex、Claude、DeepSeek、任何其他 AI 或自动化流程不得运行 Balance、自博弈统计或 ai-card-study 研究实验
- 平衡常数、权重、阈值、倍率、utility 参数及类似数值是既定规则；AI 只能读取和使用，不得依据结果调整、试探、搜索、拟合或以优化平衡为目的修改
- 所有平衡测试、结果解释与参数调整仅由项目所有者本人执行
- 正确性、回归、架构和性能测试可以执行，但不得扩展成数值平衡评估或调参建议
