...
守誓者最大生命为 4。主动技能「壁垒」消耗 2 点能量，使一名己方阵营角色（包括自己）获得 1 点护盾，每回合最多发动 2 次；该护盾遵循普通护盾的公共规则。
...
- **主动·壁垒：** 消耗 2 点能量，使一名己方阵营角色（包括自己）获得 1 点护盾。
- **主动限制：** 每回合最多发动 2 次。
...
- **主动·滋荣：** 消耗 2 点能量，使自己或一名受伤队友恢复 1 点生命。一回合最多使用 2 次。
...
- **主动·焚场：** 消耗等同于当前存活敌方角色数量的能量，对所有存活敌人各造成 1 点不可格挡伤害，一回合最多使用 2 次。
...
- **主动·孤注：** 至少拥有 1 点能量时可发动并消耗全部能量，摸取 `E - 1` 张牌，其中 `E` 为本次实际消耗的能量；有 `min(100%, E × 25%)` 的概率进入不可叠加的孤注状态，下一次突袭额外造成 1 点伤害，突袭完毕后退出孤注状态，一回合最多使用 1 次。
