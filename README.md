...
《五域纷争》是一款原创、可离线运行并支持多人联机的五人阵营卡牌游戏。玩家可选择单机模式，与四名电脑角色共同对局；也可通过局域网或虚拟局域网进行多人联机，由真人玩家与电脑角色共同组成五人牌局。五名角色会被公开随机分为 2 人和 3 人阵营，在环形桌面上通过基础牌、战术牌、装备、角色技能、即时响应和濒死救援消灭敌方全部角色。

项目主体使用 HTML、CSS、原生 JavaScript、ES Module 和浏览器 API，并通过 Electron 提供桌面运行环境及基于 TCP 的多人联机 Transport。游戏规则、AI 与界面运行不依赖在线服务、在线素材或运行时第三方业务依赖；角色肖像与二十八种卡牌插画均为项目内原创 SVG。
...
守誓者最大生命为 4。主动技能「壁垒」消耗 2 点能量，使一名己方阵营角色获得 1 点护盾，每回合最多发动 2 次；该护盾遵循普通护盾的公共规则。

## 八名角色

角色的背景势力、标签和人物故事用于塑造身份与战术风格，不决定其本局阵营。每局征召后，角色按随机座次进入固定两人的晨星或固定三人的暮影阵营。

### 刃行者

- **背景势力／生命／初始能量：** 流火长路／4 点生命／0 点能量。
- **定位：** 输出、爆发。擅长安排不同类别的出牌顺序，把资源牌和战术牌转化为下一次突袭的爆发伤害。
...
- **背景势力／生命／初始能量：** 曜石城垒／4 点生命／0 点能量。
- **定位：** 防护、辅助。能在伤害落下前保护队友，并用可叠加的护盾维持阵线。
- **人物描述：** 背负古誓的城垒守卫，擅长将危险拦截在盟友身前。
- **被动·护援：** 队友即将受到伤害时，可以弃置 1 张手牌令该次伤害减少 1 点；每回合限触发 1 次。
- **主动·壁垒：** 消耗 2 点能量，使一名己方阵营角色获得 1 点护盾。
- **主动限制：** 每回合最多发动 2 次。

### 灵医

- **背景势力／生命／初始能量：** 雾泉庭／4 点生命／1 点能量。
- **定位：** 恢复、辅助。通过每回合前两次实际治疗获得额外手牌，也能以较低能量连续救治受伤角色。
- **人物描述：** 听见生命回声的游医，以自己的元息换取同伴继续作战。
- **被动·回春：** 由你使一名己方阵营角色恢复生命时，摸 1 张牌；濒死救援也可以触发；每回合最多触发 2 次。
- **主动·滋荣：** 消耗 2 点能量，使一名己方阵营角色恢复 1 点生命。
- **主动限制：** 一回合最多使用 2 次。

### 影客

- **背景势力／生命／初始能量：** 无灯港／4 点生命／2 点能量。
- **定位：** 控制、辅助。通过伤害建立私密手牌情报，再直接窃取敌人的隐藏资源。
- **人物描述：** 往返暗潮市集的情报客，相信一张被看见的牌就不再是秘密。
- **被动·窥隙：** 对敌人造成实际伤害后，私下选择并查看该敌人至多 2 张手牌。
- **主动·窃取：** 消耗 2 点能量，选择一名持有手牌或装备的敌人，将其全部手牌与装备区牌组成统一候选集合，等概率随机获得其中 1 张并收入手牌；每回合最多发动 2 次。

### 炎术师

- **背景势力／生命／初始能量：** 赤砂穹庐／4 点生命／1 点能量。
- **定位：** 群攻、爆发。持续把卡牌造成的伤害转化为能量，并用焚场压低整个敌方阵营的生命。
- **人物描述：** 以赤砂为燃料的术士，能从每一道伤痕里回收燃烧的余烬。
- **被动·余烬：** 一张卡牌的结算中首次对敌人造成实际伤害后获得 1 点能量，每次卡牌结算最多触发 1 次。
- **主动·焚场：** 消耗 3 点能量，对所有存活敌人各造成 1 点可格挡伤害，一回合最多使用 2 次。

### 追猎者

- **背景势力／生命／初始能量：** 苍苔原／4 点生命／0 点能量。
- **定位：** 突破、爆发。先用普通突袭留下猎印，再在之后的行动中发动猎杀完成定点打击。
- **人物描述：** 从不追赶脚步，只追赶选择；一旦落印，猎物便难逃终局。
- **被动·追踪：** 每回合以「突袭」指定敌人后可留下「猎印」，限触发 2 次；同一名敌人一回合最多留下 1 次「猎印」；「猎印」持续到追猎者自己的下回合结束。
- **主动·猎杀：** 消耗 2 点能量，对一名带有自己猎印的敌人造成 2 点可格挡伤害，并移除该猎印，猎杀无视距离，若敌人抵挡成功，那么摸一张手牌，一回合最多使用 2 次。

### 赌命者

- **背景势力／生命／初始能量：** 镜轮市／4 点生命／0 点能量。
- **定位：** 输出、爆发。通过战术牌争取额外收益，并把积累的能量一次押在过牌与突袭强化上。
- **人物描述：** 在镜轮赌局中输掉姓名的旅人，习惯把剩下的一切推向桌面中央。
- **被动·冒险：** 每回合首次使用战术牌后，有 60% 概率摸 1 张牌。每回合限触发 1 次。
- **主动·孤注：** 至少拥有 1 点能量时可发动并消耗全部能量，摸取 `E - 1` 张牌，其中 `E` 为本次实际消耗的能量；有 `E × 25%` 的概率进入不可叠加的孤注状态，下一次突袭额外造成 1 点伤害，突袭完毕后退出孤注状态，一回合最多使用 1 次。

### 调律师

- **背景势力／生命／初始能量：** 鸣风塔／4 点生命／0 点能量。
- **定位：** 过牌、辅助。通过对队友使用卡牌形成额外循环，并以共鸣为自己或队友补充手牌。
- **人物描述：** 借风塔谐振器校准队友的行动，让每一次协作都产生新的回响。
- **被动·协调：** 每回合首次令另一名队友成为卡牌的有效作用目标后，你与该队友各摸1张牌。
- **主动·共鸣：** 消耗 2 点能量，使自己或一名存活队友摸 1 张牌，一回合最多使用 2 次。
...
合计：基础牌 92、战术牌 56、装备牌 17，共 165 张。每个实体具有唯一 `card.id`；配置与规则使用稳定 `definitionId`。
...
- 借势：选择一名装备区有真实装备且攻击范围内存在其他角色的其他角色；锁定其装备区中的同一实体装备，再选择其攻击范围内的一名其他角色作为第二目标（可以是借势使用者或第一目标的队友）。选择阶段不读取第一目标的手牌或普通「突袭」次数；实际响应时才检查实体「突袭」及其普通目标合法性，并忽略普通次数限制进入完整「突袭」流程。拒绝或无法使用时，锁定装备才从装备区移入借势使用者手牌。标准牌堆固定 3 张不同实体。
...
- 互利：规则目标为所有存活角色；反制窗口后展示等同存活人数的公共牌，从施牌者起按座位每人选 1 张。
- 共生：按座位令所有受伤存活角色恢复 1 点生命。
- 封印：对一名未处于「封印」状态的敌人使用，使其进入「封印」状态。其下个回合摸牌阶段前判定：为战术牌时「封印」未生效，本回合正常进行；否则「封印」生效，摸牌后跳过出牌阶段。
- 闪电：使用后进入「闪电」状态；下回合摸牌前进行判定。若结果为装备牌，受到3点伤害并移除状态；否则将状态转移给下一名未进入「闪电」状态的角色。
...
- 泡泡机：你的回合开始时，若你当前没有护盾，获得 1 点普通护盾；每个自己的回合开始仅检查一次，回合中途护盾降为 0 或中途装备时都不会立即补盾。
...
5. 每张「调息」恢复 1 点生命并触发 `dyingRescueUsed`；若灵医本回合前两次以「调息」使自己或队友实际恢复生命，「回春」会令灵医各摸 1 张牌。目标生命仍不大于 0 时重新开始一轮询问。
...
八名角色的领域定义位于 `js/domain/definitions/characters/CharacterDefinitions.js`，肖像位于 `assets/characters/`；二十八种卡牌的领域定义位于 `js/domain/definitions/cards/CardDefinitions.js`，牌组数量位于 `RulesetDefinition.js`，插画位于 `assets/cards/`。`art`、`icon`、`accent` 和 `frameStyle` 由 `js/adapters/ui/*PresentationDefinitions.js` 拥有，只用于展示，不参与规则或 AI 合法性。
...
已实现完整 2V3 对局、八名角色、二十八种牌、动态距离、负生命濒死救援、八件装备、多阶段隐藏选牌、公共牌池、深度牌序规划、可取消自然节奏、桌游 UI、键盘焦点、重开清理、规则测试和固定种子平衡模拟。
