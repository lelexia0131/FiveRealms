...
- **背景势力／生命：** 雾泉庭／3 点生命。
- **定位：** 恢复、辅助。通过强化每回合第一次对自己或队友的治疗提高资源效率，也能集中能量连续救治受伤角色。
- **人物描述：** 听见生命回声的游医，以自己的元息换取同伴继续作战。
- **被动·回春：** 每回合第一次由自己使自己或队友恢复生命时，本次治疗量 +1，并摸 1 张牌；濒死救援也可以触发。
- **主动·滋荣：** 消耗 2 点能量，使自己或一名受伤队友恢复 1 点生命；若目标不是自己，自己同样恢复 1 点生命。一回合最多使用 2 次。
...
- 雷达：受到「突袭」或「震荡」攻击前公开判定。战术牌进入弃牌堆并令此次攻击无效；基础牌进入防守者手牌后继续结算，新获得的「格挡」可立即响应；装备牌进入弃牌堆，此次攻击继续进入「格挡」、护盾和伤害结算。
...
5. 每张「调息」通常恢复 1 点生命并触发 `dyingRescueUsed`；若灵医本回合首次以「调息」救援自己或队友，「回春」会令本次治疗量 +1 并摸 1 张牌。目标生命仍不大于 0 时重新开始一轮询问。
