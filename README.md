...
搜索会推演卡牌消耗、突袭/调息次数、能量、四种装备、破势、格挡/反制概率、挑衅、决斗、互利、共生与濒死救援期望。反制按真实座位规则考虑全部非施牌者，并按各自阵营净收益估计响应意愿；防御装置会计入判定得到格挡并立即使用的概率。模拟中的 `alive` 始终是离散规则：预计调息足量才以 1 血获救，否则直接进入阵亡分支，`survivalChance` 只参与评分。深层节点也会生成主动技能，因此能够发现“先聚能，再发动主动技能”；结束出牌和重复装备评分读取当前模拟节点，而不是根节点的旧玩家状态。每执行一个真实动作后重新构造可见状态并重规划，旧目标不会被盲目复用。
