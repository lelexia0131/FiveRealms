...
- **背景势力／生命：** 无灯港／4 点生命。
- **定位：** 控制、谍报。通过伤害建立私密手牌情报，并窃取敌人的手牌或公开装备。
- **人物描述：** 往返暗潮市集的情报客，相信一张被看见的牌就不再是秘密。
- **被动·窥隙：** 每回合首次对敌人造成实际伤害后，私下查看该敌人至多 2 张随机手牌。
- **主动·窃取：** 消耗 2 点能量，随机获得距离 2 内一名敌人的 1 张手牌或装备牌；装备牌进入自己的手牌，一回合最多使用 1 次。
...
- **被动·协调：** 每回合首次对另一名队友使用卡牌后，自己摸 1 张牌。
...
普通突袭要求敌对、存活、非自身且 `DistanceSystem.getDistance(game, source, target) <= source.attackRange`。转移的来源与接收者都必须在施牌者距离 1 内；掠夺和影客的窃取范围为 2；破坏不限距离。震荡、挑衅、窥探、破坏、决斗、互利和共生显式忽略距离。主动技能通过 `rangeRule: "attack" | "fixed" | "unlimited" | "ally" | "self"` 声明距离语义，不从 `targetType` 猜测。
...
| 装备 | recycleDevice | 回收站 | 2 |
| 装备 | defenseDevice | 雷达 | 2 |
| 装备 | battleDevice | 军火库 | 2 |
| 装备 | telescope | 望远镜 | 2 |
| 装备 | barrierDevice | 屏障 | 2 |

合计：基础牌 68、战术牌 40、装备牌 12，共 120 张。每个实体具有唯一 `card.id`；配置与规则使用稳定 `definitionId`。

## 主要卡牌结算

- 突袭：对动态距离内敌人造成 1 点可格挡伤害；二人阵营可主动使用两次。
- 调息：两个阵营在出牌阶段均可不限次数自疗 1，但每次仍需持有实体牌且生命未满；濒死窗口可救自己或存活队友，救援不计主动次数。
- 格挡：只在突袭响应窗口使用；军火库会把需求从 1 张提高为原子弃置 2 张。
- 充能：获得 1 点能量，不超过上限。
- 望远镜：装备者计算与其他角色的距离时 -1，最低仍为 1；可让原本距离 2 的敌人进入普通突袭范围。
- 屏障：其他角色计算与装备者的距离时 +1；装备者主动计算距离时不受影响。
- 护盾：选择自己或任意一名存活队友，使其获得 1 点护盾；每点护盾免疫 1 点伤害并随之消耗，可叠加且不会在回合开始时归零。护盾是独立资源，不属于角色状态；“失去生命”效果仍会绕过护盾。
- 窥探：选择目标和最多两张背面实体牌，结果只在施牌者私密层展示。
- 转移：依次选择距离 1 内的来源、距离 1 内的不同接收者和一张实体牌。手牌转入接收者手牌；装备区的牌直接转入接收者装备区，并按唯一装备槽规则替换其原装备。未知手牌仍显示无文字、无编号的纯牌背。
...
- 掠夺：选择距离 2 内一名其他角色的手牌或公开装备；取得的装备牌进入施牌者手牌。真人亲自取得或原本已知的实体牌会继续显示并记录牌名，AI 之间未公开取得的手牌仍只记录“1张手牌”。
- 破坏：不限距离，公开并弃置一名其他角色的指定手牌或装备牌，并在中央短暂展示。
- 反制：任何其他存活角色可响应尚未生效的主动战术牌，反制本身也能继续被反制；面对震荡时只取消对当前响应目标的效果。
- 收获：直接摸 2 张牌。
- 决斗：目标先开始，双方轮流弃突袭；先放弃或无牌者受到 1 点普通伤害。
- 互利：反制窗口后展示等同存活人数的公共牌，从施牌者起按座位每人选 1 张；真人先选中卡牌，再点击“确定”提交，正常结算中不能取消或跳过。
...
- 雷达：普通或震荡突袭前进入公开判定区。战术牌令攻击免疫并弃置判定牌；基础牌归目标手牌后继续攻击，新获得的格挡可立即响应；装备牌令目标直接失去 1 生命并终止原攻击。
