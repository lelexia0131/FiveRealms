...
AI 的救援职责判断使用稳定的英文 `roleTags`，不依赖界面显示的中文标签。多人协作救援只读取后续队友的公开手牌数量与自己合法记住的牌，计算当前一张救援后仍缺几张及后续可能贡献；不会借此读取队友或敌人的真实隐藏手牌。濒死调息统一调用 `Game.heal()`，因此保留治疗事件、统计和动画，但 `isDyingRescue` 上下文会阻止灵医“回春”额外放大救援。
...
搜索会推演卡牌消耗、突袭/调息次数、能量、四种装备、破势、格挡/反制概率、挑衅、决斗、互利、共生与濒死救援期望。深层节点也会生成主动技能，因此能够发现“先聚能，再发动主动技能”；未知响应概率只从牌数、公开记忆与牌组构成估算。每执行一个真实动作后重新构造可见状态并重规划，旧目标不会被盲目复用。展开每 48 个节点通过 `CleanupManager.delay(0)` 让出事件循环；达到预算立即返回当前最佳方案。
