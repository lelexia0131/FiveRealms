/*
 * 模块职责：把 GameState 投影为只含公开事实与观察者自身私有信息的不可变 VisibleState。
 * 上游调用方：AI 状态组合入口、状态契约测试。
 * 下游依赖：无。
 * 状态边界：只读 GameState，创建新的 VisibleState 快照。
 * 信息边界：允许公开区域与观察者自己的手牌；禁止敌方未知手牌与 AI 记忆。
 * 架构约束：不得计算概率、价值、搜索分支，也不得保留 Game 或 Player 引用。
 */

/*
 * 功能：创建不可变的普通值数组，切断快照与 GameState 集合的引用。
 * 调用方：createVisibleState、projectVisiblePlayer。
 * 输入：只含普通值或已投影对象的数组。
 * 输出：新的冻结数组。
 * 读取状态：无。
 * 写入状态：无。
 * 调用函数：Object.freeze。
 * 边界与不变量：输入内容必须已完成信息过滤，不负责深层投影。
 */
...
/*
 * 功能：把合法可见卡牌压缩为不含运行时反向引用的实体标识。
 * 调用方：createVisibleState、projectVisiblePlayer。
 * 输入：公开卡牌或观察者自己的手牌实体。
 * 输出：冻结的卡牌 ID 与定义 ID 对象。
 * 读取状态：卡牌 id、definitionId。
 * 写入状态：无。
 * 调用函数：Object.freeze。
 * 边界与不变量：不得复制牌面之外的可变运行时字段。
 */
...
/*
 * 功能：把单个玩家投影为公开事实，并只为观察者本人携带手牌牌面。
 * 调用方：createVisibleState。
 * 输入：观察者 ID 与 GameState 中的 Player。
 * 输出：冻结的 VisibleState 玩家记录。
 * 读取状态：Player 身份、公开属性、回合标记、状态与本人手牌。
 * 写入状态：无。
 * 调用函数：projectVisibleCard、freezeList。
 * 边界与不变量：非观察者只暴露 handCount，绝不访问其手牌定义进行投影。
 */
...
/*
 * 功能：创建指定观察者的不可变 VisibleState 快照。
 * 调用方：createAiStateContracts、状态契约测试。
 * 输入：合法玩家 ID 与当前 GameState。
 * 输出：不含敌方未知信息、概率和搜索字段的冻结快照；观察者缺失时抛错。
 * 读取状态：GameState 回合、牌区公开计数、公开卡牌与玩家事实。
 * 写入状态：无。
 * 调用函数：projectVisibleCard、projectVisiblePlayer、freezeList。
 * 边界与不变量：投影结果不得因敌方未知手牌定义变化而变化，也不得保留 GameState 引用。
 */
