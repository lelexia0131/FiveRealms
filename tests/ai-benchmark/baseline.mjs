 ...
 *
 * 1. Random Legal：
 *    - 只读取生产合法动作集合（ActionGenerator.getActionCandidates）；
 *    - 从合法动作中均匀随机选择（deterministic seed）；
 *    - 禁止调用生产 evaluator、simulator、planner 或任何估值函数。
 *
 * 2. Greedy / One-Step：
 *    - 允许使用生产 AiEvaluator + Simulator；
 *    - 只做"当前状态 → 每个合法动作 → 一步评估"；
 *    - 禁止调用 Planner / AIController.selectAction（无搜索、无前瞻）。
 *
 * 3. Production AI：
 *    - AIController → Planner → Simulator → AiEvaluator 原样调用；
 *    - 不修改生产代码。
 *
 * 信息公平：三个 Agent 都通过 game.aiController.getActionCandidates 获得
 * 同一合法动作集合；Greedy 通过生产 createInitialSearchState 获得与
