...
- 本工具及其 corpus、experiment、windows、aggregate、interim、recovery 等入口仅允许项目所有者本人手动运行和解释。
- 禁止 Codex、Claude、DeepSeek、任何其他 AI 或自动化流程运行本工具、Balance 测试或其他 AI 自博弈统计。
- 禁止根据本工具或其他测试结果调整、试探、搜索、拟合平衡常数、权重、阈值、倍率、utility 参数或类似数值。
- 平衡常数只能作为既定规则读取和使用，不得以优化平衡、改善胜率或匹配统计目标为目的修改。
- AI 可以执行正确性、回归、架构和性能测试，但不得把这些测试扩展为数值平衡评估，也不得从研究报告生成调参建议。
...
聚合（`lib/aggregate.js`）输出每张卡的 ΔWR、95% CI、按 HP/手牌/能量/局势/阶段分层的均值和按角色的价值矩阵；另有对“当前 `aiValue` 是否高估/低估”的提示。该提示仅供项目所有者本人研究，AI 或自动化流程不得据此提出或实施数值修改。
...
以下命令全部仅供项目所有者本人手动执行，不得由 AI、代理 Worker 或自动化测试调用。
...
本目录是项目所有者专属的独立研究工具，不加入 `tests/run.mjs`；`tests/run.mjs` 继续作为快速回归测试。研究类长 benchmark 不进入常规单元测试，也不得被 AI 或自动化流程作为平衡评估入口调用。
