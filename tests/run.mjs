...
test("AI 转移敌方到己方已知与未知期望值4混合时平局优先已知", () => { const actor=makePlayer("actor",0,"dawn"),from=makePlayer("from",1,"dusk"),receiver=makePlayer("receiver",2,"dawn");const {game}=makeGame([actor,from,receiver],{random:()=>0});const known=instance("charge"),unknown=instance("counter");let reads=0;const definitionId=unknown.definitionId,aiValue=unknown.aiValue;Object.defineProperty(unknown,"definitionId",{configurable:true,get(){reads+=1;return definitionId;}});Object.defineProperty(unknown,"aiValue",{configurable:true,get(){reads+=1;return aiValue;}});from.hand=[known,unknown];game.rememberPrivateCard(actor,from,known);const chosen=game.aiController.cardSelector.chooseHiddenCards(actor,from,1,null,{purpose:"transfer",receiver})[0];assert.equal(reads,0);assert.equal(chosen,known); });
...
test("转移评分预计牌值对已知与未知混合使用期望值4", () => { const actor=makePlayer("actor",0,"dawn"),from=makePlayer("from",1,"dusk"),receiver=makePlayer("receiver",2,"dawn");const {game}=makeGame([actor,from,receiver]);const low=instance("charge"),high=instance("counter"),unknown=instance("assault");from.hand=[low,high,unknown];game.rememberPrivateCard(actor,from,low);game.rememberPrivateCard(actor,from,high);assert.equal(expectedHandValue(actor,from,"highest"),8);assert.equal(expectedHandValue(actor,from,"lowest"),4); });
