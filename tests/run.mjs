...
test("AI 普通突袭与借势响应共用同一模拟入口", () => {
  ...
  const simulator=new AiSimulator(state),original=simulator.simulateAssault.bind(simulator),sources=[];simulator.simulateAssault=(next,source,target,chance)=>{sources.push(source.id);return original(next,source,target,chance);};simulator.apply(state,{type:"card",card:{...CARD_DEFINITIONS.assault,id:"a"},targets:[{id:"first"}]},"actor");simulator.apply(state,{type:"card",card:{...CARD_DEFINITIONS.leverage,id:"l"},targets:[{id:"first"},{id:"second"}],selection:{firstTargetId:"first",secondTargetId:"second"}},"actor");assert.deepEqual(sources,["actor","first"]);
});
