...
test("两个阵营 UI 都把主动调息上限显示为∞", () => { const {small,large}=makeTeamFixture();assert.match(playerPanelTemplate(small,{isHuman:true}),/调息 0\/∞/);assert.match(playerPanelTemplate(large,{isHuman:true}),/调息 0\/∞/); });
...
test("真人没有格挡时仍出现响应窗口，但不能凭空格挡", async () => { const a=makePlayer("a",0,"dawn"),b=makePlayer("b",1,"dusk","human");const {game,ui}=makeGame([a,b],{response:()=>true});const hp=b.hp;await game.damage(a,b,1,{card:instance("assault"),canBlock:true,damageType:"normal"});assert.equal(ui.responseRequests.length,1);assert.deepEqual(ui.responseRequests[0].legalCardIds,[]);assert.equal(ui.responseRequests[0].requiredCount,1);assert.equal(b.hp,hp-1); });
